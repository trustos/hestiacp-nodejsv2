...
class NodeJsPaths
{
    ...
    private const APP_ENTRYPOINT_NAME_ESM = "ecosystem.config.js";
    private const APP_ENTRYPOINT_NAME_CJS = "ecosystem.config.cjs";
    private const APP_ENTRYPOINT_TEMPLATE =
        __DIR__ . "/../templates/web/entrypoint.tpl";
    ...
    private const NODEJS_PROXY_CONFIG_TEMPLATE =
        __DIR__ . "/../templates/nginx/nodejs-app.tpl";
    private const NODEJS_PROXY_FALLBACK_CONFIG_TEMPLATE =
        __DIR__ . "/../templates/nginx/nodejs-app-fallback.tpl";
    ...
    public function getAppEntryPoint(
        string $domain,
        bool $isCommonJS = false
    ): string {
        return $this->getAppDir(
            $domain,
            $this->getAppEntryPointFileName($isCommonJS)
        );
    }

    public function getAppEntryPointFileName(bool $isCommonJS = false): string
    {
        return $isCommonJS
            ? self::APP_ENTRYPOINT_NAME_CJS
            : self::APP_ENTRYPOINT_NAME_ESM;
    }
    ...
    public function getAppProxyFallbackConfig(string $domain): string
    {
        return $this->getDomainConfigDir(
            $domain,
            self::APP_PROXY_FALLBACK_CONFIG_FILE_NAME
        );
    }
